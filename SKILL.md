...
**Required**: a community profile in `profiles/`, a member export or platform API read, and enrichment reach — Context.dev through Opulent's native integration, `web_search`/`web_fetch`, `browser_*` as last resort. Lane two additionally needs the ICP and exclusion blocks filled in.
...
- **Cheapest first**: platform export/API → Context.dev → `web_search`/`web_fetch` → `browser_*` only for dynamic or authenticated surfaces. One retrieval per unique identity per cycle; reuse across the cohort.
...
## CRITICAL — Writeback and invitations are gated
...
| Writeback to platform API | `ask_question`, once, with field-level counts and the dry-run diff |
| Invitation handoff | `ask_question` per batch, after suppression, with the prospect list visible |
| Direct outreach by this skill | **Never.** The platform's existing automated emails send; this skill only queues |
...
Produces `refresh.html` (changes first, grouped by member; quiet members collapsed), `changes.csv`, `writeback.json` — the API-ready payload, every field carrying value + provenance + confidence — and `summary.json`. Surface the table with `create_spreadsheet_artifact`, then gate:
...
Register the recurring run with `automation_manage` on the profile's cadence. The cycle is idempotent end to end — a retry after a crash re-emits nothing.
